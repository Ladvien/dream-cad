...
class BenchmarkRunner:
    ...
    def run_comprehensive_benchmark(
        ...
        if prompt_category == "all":
            ...
            for category_prompts in STANDARD_PROMPTS.values():
                prompts.extend(category_prompts[:1])
        else:
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> Dict[str, str]:
        ...
        for name, metrics in models.items():
            ...
            speed_score = 1 - min(metrics["avg_generation_time"] / 60, 1)
            memory_score = 1 - min(metrics["avg_vram_gb"] / 24, 1) if metrics["avg_vram_gb"] > 0 else 0.5
