...
class CostReport:
    ...
    model_cost_breakdown: Dict[str, Dict[str, float]]
    ...
    hourly_cost_distribution: Dict[int, float]
...
class CostAnalyzer:
    ...
    def _calculate_storage_costs(
        ...
        storage_gb = (num_generations * 100) / 1024
        ...
        if model_analysis["model_costs"]:
            ...
            for model, cost in model_analysis["model_costs"].items():
                if cost > avg_cost * 1.5:
                    ...
                    potential_savings += (cost - avg_cost) * 0.3
