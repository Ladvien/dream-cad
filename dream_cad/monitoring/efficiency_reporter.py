"""Efficiency reporting for multi-model 3D generation system."""
...
class EfficiencyReport:
    ...
    model_rankings: Dict[str, float]  # Model -> efficiency score
    most_efficient_model: str
    least_efficient_model: str
    
    # Resource efficiency
    vram_efficiency: Dict[str, float]  # Model -> VRAM efficiency %
    gpu_efficiency: Dict[str, float]  # Model -> GPU efficiency %
    time_efficiency: Dict[str, float]  # Model -> time efficiency %
    
    # Quality vs resource tradeoffs
    quality_per_vram: Dict[str, float]  # Quality score per GB VRAM
    quality_per_second: Dict[str, float]  # Quality score per second
    quality_per_dollar: Dict[str, float]  # Quality per estimated cost
    
    # Optimization opportunities
    optimization_opportunities: List[Dict[str, Any]]
    potential_savings: Dict[str, float]  # Resource -> potential savings
    
    # Best practices
    recommended_configs: Dict[str, Dict[str, Any]]  # Model -> optimal config
    batch_size_recommendations: Dict[str, int]
    
    # Comparative analysis
    model_comparisons: List[Dict[str, Any]]  # Pairwise comparisons
    use_case_recommendations: Dict[str, str]  # Use case -> recommended model
...
class EfficiencyReporter:
    """Generate efficiency reports for model optimization."""
    
    def __init__(
        self,
        storage_dir: Optional[Path] = None,
    ):
        """Initialize efficiency reporter.
    ...
    def _calculate_efficiency_scores(
        ...
        """Calculate overall efficiency scores for models.
        ...
        for model_name, stats in model_stats.items():
            ...
            # Speed efficiency (inverse of generation time)
            avg_time = stats.get("avg_generation_time", float('inf'))
            if avg_time > 0:
                speed_score = min(100, 60 / avg_time * 100)  # 60s as baseline
            else:
                speed_score = 0
            
            # Resource efficiency (inverse of VRAM usage)
            avg_vram = stats.get("avg_vram_gb", float('inf'))
            if avg_vram > 0:
                resource_score = min(100, 8 / avg_vram * 100)  # 8GB as baseline
            else:
                resource_score = 0
            ...
            efficiency = (
                success_rate * 100 * 0.3 +  # 30% weight on reliability
    ...
    def _analyze_resource_efficiency(
        ...
        """Analyze resource efficiency for each model.
        ...
        # Find baselines (best in each category)
        min_vram = min(
        ...
        for model_name, stats in model_stats.items():
            # VRAM efficiency (how well it uses allocated VRAM)
    ...
    def _calculate_quality_tradeoffs(
        ...
        """Calculate quality per resource unit.
        
        Args:
            model_stats: Statistics for each model
            
        Returns:
            Quality tradeoff metrics
        """
        per_vram = {}
        per_second = {}
        per_dollar = {}
        ...
        for model_name, stats in model_stats.items():
            ...
            # Estimated cost (simplified: VRAM hours * $0.10)
            cost = (vram * time / 3600) * 0.10  # $0.10 per GB-hour
    ...
    def _find_optimization_opportunities(
        ...
        Args:
            model_stats: Statistics for each model
            efficiency_scores: Calculated efficiency scores
        ...
        for model_name, stats in model_stats.items():
            ...
            if stats.get("avg_vram_gb", 0) > 16:
                opportunities.append({
                    ...
                    "potential_improvement": 20,  # Estimated 20% reduction possible
                })
            ...
            if stats.get("avg_generation_time", 0) > 120:
                opportunities.append({
                    ...
                    "potential_improvement": 30,  # Estimated 30% speedup possible
                })
    ...
    def _generate_recommendations(
        ...
        for model_name, stats in model_stats.items():
            # Optimal configuration based on performance
            config = {}
    ...
    def _compare_models(
        ...
        # Sort by absolute efficiency difference
        comparisons.sort(
            key=lambda x: abs(x["efficiency_diff"]),
            reverse=True,
        )
        
        return comparisons[:20]  # Top 20 comparisons
    ...
    def _calculate_potential_savings(
        ...
        Returns:
            Potential savings by resource type
        """
        savings = {
            "vram_gb": 0,
            "time_hours": 0,
    ...
    def _save_report(self, report: EfficiencyReport) -> None:
        """Save efficiency report to disk.
        
        Args:
            report: Efficiency report to save
        """
        try:
