#!/usr/bin/env python3
"""Demonstrate the multi-model 3D generation system."""
...
def demonstrate_system():
    """Demonstrate the complete 3D generation system."""
    ...
    # Register our models (in case registry is empty)
    models_info = {
        "triposr": {
            "min_vram_gb": 4.0,
    ...
    for model_name, capabilities in models_info.items():
        # Registry auto-discovers models, just display info
        print(f"  • {model_name}:")
    ...
    # 2. Check hardware and get recommendations
    print("\n🖥️ Hardware Check:")
    print("-" * 40)
    
    # Simulate available VRAM (would normally detect from GPU)
    available_vram = 24.0  # RTX 3090
    print(f"  Available VRAM: {available_vram}GB")
    
    # Show which models would work with this VRAM
    compatible_models = []
    ...
    # 4. Test model creation and generation
    print("\n🎨 Testing Model Generation:")
    print("-" * 40)
    
    # Test with TripoSR (fastest model)
    model_name = "triposr"
    ...
    # Start monitoring
    config = {"device": "cpu", "model_name": model_name}  # Using CPU for demo
    ...
    try:
        ...
        # End monitoring with mock success
        metrics = dashboard.end_generation(
            model_name,
            success=True,
