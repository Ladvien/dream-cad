...
class WizardScreen(Screen):
    ...
    def on_progress_update(self, message):
        if hasattr(self, 'current_job') and message.job_id == self.current_job.id:
    ...
    def on_job_complete(self, message):
        if hasattr(self, 'current_job') and message.job.id == self.current_job.id:
            ...
            if message.job.status == JobStatus.COMPLETED:
                ...
                if self.app.screen.id == "dashboard":
                    dashboard = self.app.screen
                    dashboard.on_job_complete(message)
